...
static int json_parse_value(struct json_parse_state_s* state,
  ...
  case '"':
    ...
    state->dom += sizeof(struct json_string_s);
    return json_parse_string(state, (struct json_string_s *)(value->payload));
  case '{':
    value->type = json_type_object;
    value->payload = state->dom;
    state->dom += sizeof(struct json_object_s);
    return json_parse_object(state, (struct json_object_s *)(value->payload));
  case '[':
    value->type = json_type_array;
    value->payload = state->dom;
    state->dom += sizeof(struct json_array_s);
    return json_parse_array(state, (struct json_array_s*)(value->payload));
  case '-':
  case '0':
  case '1':
  case '2':
  ...
  case '9':
    ...
    return json_parse_number(state, (struct json_number_s*)value->payload);
  default:
    if ((state->offset + 4) < state->size &&
      't' == state->src[state->offset + 0] &&
      'r' == state->src[state->offset + 1] &&
...
#endif // #GLMC_JSON_IMPLEMENTATION
